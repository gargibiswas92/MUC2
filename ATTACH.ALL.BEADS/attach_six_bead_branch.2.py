...
#        if m != num and m != (num+1) and m != (num-1) and m != (num+2) and m != (num-2) and m != (num+3) and m != (num-3) and m != (num+4) and m != (num-4):
        if m != num and m != (num+1) and m != (num-1) and m != (num+2) and m != (num-2) and m != (num+3) and m != (num-3):
           xp1, yp1, zp1 = read_coordinates(m, x_cord_prot, y_cord_prot, z_cord_prot)
...
def write_for_index(outfile7, num, ind1, ind2, ind3, ind4, ind5, ind6, sug):
    sp = '{:>8d}{:>8d}{:>8d}{:>8d}{:>8d}{:>8d}{:>8d}{:>8d}{}'.format(sug, num, ind1, ind2, ind3, ind4, ind5, ind6, '\n')
...
for k in range(count_prot_s):
    ...
    xs2, ys2, zs2 = solve_coordinates(x2, y2, z2, x3, y3, z3, x33, y33, z33, 150.4, 56.7, 1.15)
    xs3, ys3, zs3 = solve_coordinates(x2, y2, z2, x3, y3, z3, x33, y33, z33, 150.5, 56.9, 1.63)
    xs4, ys4, zs4 = solve_coordinates(x2, y2, z2, x3, y3, z3, x33, y33, z33, 90.5, 115.9, 1.15)
    xs5, ys5, zs5 = solve_coordinates(x2, y2, z2, x3, y3, z3, x33, y33, z33, 90.5, 115.9, 1.63)
    
    max_val = len(res_num) 
    
    solution = [xs, ys, zs, xs1, ys1, zs1, xs2, ys2, zs2, xs3, ys3, zs3, xs4, ys4, zs4, xs5, ys5, zs5]
    fl = check_real(solution, 5000)
    if fl == 0:
        print(solution)
        val1 = calc_prot_clashes(num, xs, ys, zs, x_cord_prot, y_cord_prot, z_cord_prot, 0.38)
        ...
        val5 = calc_prot_clashes(num, xs4, ys4, zs4, x_cord_prot, y_cord_prot, z_cord_prot, 0.35)
        val6 = calc_prot_clashes(num, xs5, ys5, zs5, x_cord_prot, y_cord_prot, z_cord_prot, 0.35)
        ...
        if val1 == 0 and val2 == 0 and val3 == 0 and val4 == 0 and val5 ==0 and val6 == 0:
           ...
           x_sol, y_sol, z_sol = append_coord(xs4, ys4, zs4, x_sol, y_sol, z_sol)
           x_sol, y_sol, z_sol = append_coord(xs5, ys5, zs5, x_sol, y_sol, z_sol)
           ...
           if len(x_sol) > 6:
               ...
               d5 = calc_sugar_clashes(x_sol, y_sol, z_sol, xs4, ys4, zs4, 0.35)
               d6 = calc_sugar_clashes(x_sol, y_sol, z_sol, xs4, ys4, zs4, 0.35)
               ...
               if ds == 0:
                   ...
                   x_sol1, y_sol1, z_sol1 = append_coord(xs4, ys4, zs4, x_sol1, y_sol1, z_sol1)
                   x_sol1, y_sol1, z_sol1 = append_coord(xs5, ys5, zs5, x_sol1, y_sol1, z_sol1)
                   ...
                   write_output(outfile1, outfile2, outfile3, outfile4, outfile5, outfile6, ind+3, ind+2, ind+1, ind, xs3, ys3, zs3)
                   write_output(outfile1, outfile2, outfile3, outfile4, outfile5, outfile6, ind+4, ind+1, ind, num, xs4, ys4, zs4)
                   write_output2(outfile1, outfile2, outfile3, outfile4, outfile5, outfile6, ind+5, ind+4, ind+1, ind, xs5, ys5, zs5)
                   
                   sug_count = sug_count + 6
                   sug = sug + 1
                   
                   write_for_index(outfile7, num, ind, ind+1, ind+2, ind+3, ind+4, ind+5, sug)
